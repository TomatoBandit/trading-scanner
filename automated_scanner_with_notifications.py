...
class AutomatedSignalNotifier:
    ...
    def load_watchlist(self) -> List[str]:
        ...
        if os.path.exists(latest_path):
            print(f"📂 Loading watchlist from: {latest_path}")
            ...
            if watchlist_files:
                ...
                print(f"📂 Loading watchlist from: {latest_file}")
                ...
                print("⚠️ No integrated_watchlist files found. Using fallback symbols.")
                symbols = [
                    "SPY",
                    "QQQ",
                    "AAPL",
        ...
        print(f"📋 Watchlist symbols ({len(symbols)}): {', '.join(symbols)}")
        return symbols
    ...
    def market_is_tradable(self, vix_threshold: float = 22.0) -> bool:
        ...
        try:
            ...
            if latest_vix > vix_threshold:
                print(
                    f"⛔ VIX {latest_vix:.1f} > {vix_threshold} — "
    ...
    def scan_watchlist(self, symbols: List[str]) -> List[Dict[str, Any]]:
        ...
        for symbol in symbols:
            ...
            if df is None or df.empty:
                print(f"⚠️ Not enough data for {symbol} after signal calculation.")
                continue

            latest = df.iloc[-1]

            if not latest["Below_Threshold"]:
                print(f"  ↳ {symbol}: Not below threshold.")
                continue

            if not latest.get("Trend_OK", False):
                print(f"  ↳ {symbol}: Trend filter not passed.")
                continue

            if not latest.get("RSI_OK", False):
                print(
                    f"  ↳ {symbol}: RSI {latest['RSI_14']:.1f} not oversold enough."
                )
                continue
            ...
            if np.isnan(target_price) or target_price <= 0:
                print(f"  ↳ {symbol}: Invalid MA20 target price.")
                continue
            ...
            if risk_per_share <= 0:
                print(f"  ↳ {symbol}: Non-positive risk per share.")
                continue
            ...
            if shares <= 0:
                print(f"  ↳ {symbol}: Position size too small at this price.")
                continue

            if potential_gain_pct < self.min_profit_potential:
                print(
                    f"  ↳ {symbol}: Potential gain {potential_gain_pct:.2f}% "
            ...
            if risk_reward_ratio < self.min_risk_reward:
                print(
                    f"  ↳ {symbol}: R/R {risk_reward_ratio:.2f} < "
                    f"{self.min_risk_reward:.2f} min."
                )
                continue

            print(
                f"  ✅ {symbol}: Potential {potential_gain_pct:.2f}% "
    ...
    def build_discord_embed(self, opportunities: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for opp in opportunities:
            ...
            line = (
                f"**{sym}**  |  Price: `${cp:.2f}`  → Target: `${tp:.2f}`  "
    ...
    def send_discord_notification(self, opportunities: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            if response.status_code == 204:
                ...
                print(
                    f"⚠️ Discord notification failed: {response.status_code} "
                    f"{response.text}"
                )
        except Exception as e:
    ...
    def send_discord_no_opportunities(self) -> None:
        ...
        if not self.discord_webhook_url:
            print(
                "⚠️ DISCORD_WEBHOOK_URL not set. Skipping Discord 'no-opps' notification."
            )
            return
    ...
    def send_email_notification(self, opportunities: List[Dict[str, Any]]) -> None:
        ...
        """
        print("ℹ️ Email notification not configured. Skipping.")
    ...
    def run_automated_scan(self) -> List[Dict[str, Any]]:
        ...
        if not watchlist:
            print("⚠️ Watchlist is empty. Nothing to scan.")
            return []

        print(f"📊 Scanning {len(watchlist)} symbols from watchlist...")
